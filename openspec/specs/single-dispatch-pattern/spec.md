...
## Purpose
Establishes the single-dispatch pattern that reduces boilerplate by 80% compared to per-operation exports. Replaces N individual function exports with a unified `Command(cmd, data)` interface for routing all operations through a handler registry, with protobuf as the serialization format.
## Requirements
